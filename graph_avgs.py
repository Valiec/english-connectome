...
with open(sys.argv[1]) as f: # read in averages CSV
    for line in f:
        if line.startswith("sample,"):  # header
            continue
        l = line.split(",")
        data_x.append(float(l[1]))
        data_y.append(float(l[2]))
...
plt.legend([p1, p2, p3, p4, p5, p6, p7], ["2500 All Etym", "1000 All Etym", "2500 No Roots", "1000 No Roots/Affixes", "1000 Only Roots", "1000 No Affixes", "1000 No Roots"]) # legend
plt.show() # show graph