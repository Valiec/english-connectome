...
degree_list = the_graph.degree() # get degrees of all nodes (if your IDE says calling degree() is invalid, it's wrong, ignore it)

for node in degree_list: # output degree of each node 
	print(node[0]+","+str(node[1]))

nx.drawing.nx_pydot.write_dot(the_graph, "data/graphdata_"+sys.argv[1].replace(".csv", "").replace("data/", "")+".dot") # output dot file
