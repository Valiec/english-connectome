...
# Usage: python sample_check.py <edge list> <node list> <starting size> <ending size> <step> <plot header name> <output name>
...
for size in range(int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5])):
    ...
    with open(sys.argv[1].replace(".csv", "_sample_").replace("data/", "samples/") + str(size) + ".csv",
              "w") as f:  # output sample
